...
try {
    const interfacesData = loadJsonData(CONFIG.DATA_FILES.INTERFACES);
    const eventsData = loadJsonData(CONFIG.DATA_FILES.EVENTS);
    const messageTypesData = loadJsonData(CONFIG.DATA_FILES.MESSAGE_TYPES);
    // 创建Express应用
    const app = express();
    // 创建MCP服务器
    const createMcpServer = () => {
        ...
        // 注册工具：获取接口详情
        server.tool("get_interface_detail", {
        ...
        // 注册工具：获取所有消息类型
        server.tool("list_message_types", {
        ...
        // 注册工具：获取特定消息类型的详细信息
        server.tool("get_message_type_detail", {
