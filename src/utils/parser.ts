...
import type { EventInfo, InterfaceInfo, MessageClassInfo, MessageTypeInfo } from '../types.js';
...
export function parseWfcInterfaces(source: string): InterfaceInfo[] {
    if (!source) {
        return [];
    }
...
function dedupeByName(list: InterfaceInfo[]): InterfaceInfo[] {
...
export function extractInterfaceDocs(interfaces: InterfaceInfo[], name: string): InterfaceInfo | undefined {
    return interfaces.find(item => item.name === name);
}

export function parseEvents(source: string): EventInfo[] {
    if (!source) {
        return [];
    }
...
export type MessageConfigEntry = {
    name: string;
    typeEnum: string;
    flag: string;
    contentClazz: string;
...
export type MessageClassPlacement = {
    filePath: string;
    className: string;
    baseClass?: string;
    jsdoc?: string;
...
function extractClassMeta(source: string, filePath: string): MessageClassPlacement[] {
    ...
    const blocks: MessageClassPlacement[] = [];
    const lines = source.split(/\r?\n/);
...
function parseMessageConfig(source: string): MessageConfigEntry[] {
    if (!source) {
        return [];
    }

    const regex =
        /\{\s*name:\s*'([^']+)'\s*,\s*flag:\s*([^,]+)\s*,\s*type:\s*MessageContentType\.([A-Za-z0-9_]+)\s*,\s*contentClazz:\s*([A-Za-z0-9_]+)\s*\}/g;
    const results: MessageConfigEntry[] = [];
    let match: RegExpExecArray | null;
...
export type MessageContentTypeEntry = {
    enumName: string;
    value: number;
    jsdoc?: string;
};

export type PersistFlagEntry = {
    name: string;
    value: number;
    jsdoc?: string;
};

function parseEnumFile(source: string): MessageContentTypeEntry[] {
    if (!source) {
        return [];
    }

    const lines = source.split(/\r?\n/);
    const entries: MessageContentTypeEntry[] = [];
...
function parsePersistFlags(source: string): PersistFlagEntry[] {
    if (!source) {
        return [];
    }

    const lines = source.split(/\r?\n/);
    const results: PersistFlagEntry[] = [];
...
export type MessageMetaOptions = {
    messageConfigPath: string;
    messageDir: string;
    messageContentTypePath: string;
    persistFlagPath: string;
};

export function parseMessageMeta({
    messageConfigPath,
    messageDir,
    messageContentTypePath,
    persistFlagPath
}: MessageMetaOptions): MessageTypeInfo[] {
    ...
    const classMetaMap = new Map<string, MessageClassInfo>();
